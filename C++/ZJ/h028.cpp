...
bool check ( int idx ) {
	return lib[idx].pos - lib[idx].h >= lib[lib[idx].prev].pos || lib[idx].pos + lib[idx].h <= lib[lib[idx].next].pos;
}
...
int main() {
	...
	lib[0].pos = 0, lib[0].h = 0;
	lib[n + 1].pos = l, lib[n + 1].h = 0;
	for ( int i = 0 ; i <= n ; i++ )
		lib[i].prev = i - 1, lib[i].next = i + 1;
	...
	for ( int i = 1 ; i <= n ; ) {
		if ( check ( i ) ) {
			ans++, ma = max ( lib[i].h, ma );
			lib[lib[i].prev].next = lib[i].next;
			lib[lib[i].next].prev = lib[i].prev;
