...
void update ( int n ) {
	if ( graph[n].ls == -1 && graph[n].rs == -1 )
		return;
	update ( graph[n].ls );
	update ( graph[n].rs );
	graph[n].value = graph[graph[n].ls].value + graph[graph[n].rs].value;
}

int insert ( int n, int value ) {
	if ( graph[n].ls == -1 && graph[n].rs == -1 ) {
		graph[n].value += value;
		return n;
	}
	int res;
	if ( graph[graph[n].ls].value <= graph[graph[n].rs].value )
		res = insert ( graph[n].ls, value );
	else
		res = insert ( graph[n].rs, value );
